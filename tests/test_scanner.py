import pytest
from src.veda import Scanner, TokenType, Token, token_type
...
def test_scan_empty_string():
    ...
    assert all([
        len(tokens) == 1, 
        _token_equal(tokens[0], TOKEN_EOF)
    ])
...
def test_scan_left_paren():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_LEFT_PAREN)
    ])
...
def test_scan_right_paren():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_RIGHT_PAREN)
    ])
...
def test_scan_left_brace():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_LEFT_BRACE)
    ])
...
def test_scan_right_brace():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_RIGHT_BRACE)
    ])
...
def test_scan_comma():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_COMMA)
    ])
...
def test_scan_dot():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_DOT)
    ])
...
def test_scan_minus():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_MINUS)
    ])
...
def test_scan_plus():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_PLUS)
    ])
...
def test_scan_semicolon():
    ...
    assert all([
        len(tokens) == 2, 
        _token_equal(tokens[0], TOKEN_SEMICOLON)
    ])
...
def test_scan_float_with_semicolon():
